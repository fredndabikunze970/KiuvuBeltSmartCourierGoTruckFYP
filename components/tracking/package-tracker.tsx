...
export function PackageTracker({ trackingId }: PackageTrackerProps) {
  ...
  const [allowTrackingWithoutPayment, setAllowTrackingWithoutPayment] =
  ...
  const fetchTrackingData = async (trackingId: string) => {
    ...
    try {
      ...
      setCurrentLocation((response as any).currentLocation || null);
      setEstimatedTime((response as any).estimatedTime || null);

      const newProgress = (response as any).progress || 0;
      ...
      saveProgressState({
        ...
        currentLocation: (response as any).currentLocation,
        distanceTraveled: (response as any).distanceTraveled,
        distanceRemaining: (response as any).distanceRemaining,
        estimatedArrival: (response as any).estimatedArrival,
      });

      setPaymentConfirmed((response as any).paymentConfirmed ?? true);
      setAllowTrackingWithoutPayment(
        (response as any).allowTrackingWithoutPayment ?? true
      );
      setOriginBranch((response as any).originBranch || null);
      setDestinationBranch((response as any).destinationBranch || null);
      setRoutePolyline((response as any).routePolyline || null);
      setRouteDistance((response as any).routeDistance || null);
      setLocationHistory((response as any).locationHistory || []);
    } catch (err) {
      setError(
  ...
  useEffect(() => {
    // Clear expired progress on mount
    clearExpiredProgress();
  ...
  useEffect(() => {
    ...
    const forcedVehicleId =
      process.env.NEXT_PUBLIC_FORCE_VEHICLE_ID || "CAR001";
    ...
    console.log(
      "🔥 Setting up Firebase real-time updates for vehicle:",
      vehicleId
    );

    // Update helper
    const setLatestLocation = async (
      lat: number,
      lng: number,
      rawTs?: any,
      additionalData?: any
    ...
    onValue(
      ...
      (snapshot) => {
        ...
        if (v) {
          ...
          if (latitude !== undefined && longitude !== undefined) {
            console.log("📡 current_location update", {
              vehicleId,
              lat: latitude,
              lng: longitude,
              ts: timestamp,
            ...
            console.warn(
              "📡 current_location update failed - invalid coordinates",
    ...
    onValue(
      ...
      (snapshot) => {
        ...
        if (
          ...
          console.log("📦 tracking/<packageId> update", {
    ...
    onValue(
      ...
      async (snapshot) => {
        ...
        if (Array.isArray(data)) {
          ...
          const rawLocations = Object.keys(data).map((key) => ({
            ...data[key],
            key,
          }));
          allLocations = rawLocations.map((loc: any) => ({
            ...loc,
            _ts: normalizeTimestamp(
              loc.timestamp ?? loc.time ?? loc.created_at ?? loc.key
            ),
          }));
        }
        ...
        allLocations = allLocations
          .filter(
            (loc: any) =>
              ...
              isPlausibleTimestamp(loc._ts)
          )
          .sort((a: any, b: any) => b._ts - a._ts);

        console.log("📜 Processed history locations:", {
    ...
    const metaRefresh = setInterval(async () => {
      await fetchTrackingData(packageData.package_id);
    }, 15000);
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-700 rounded-xl p-6 text-white print:hidden">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg text-blue-100 mb-6">
            Enter your package ID to track its current location and status in
            real-time
          </p>
      ...
      {packageData && !allowTrackingWithoutPayment && (
        <Alert className="rounded-lg border-yellow-500 bg-yellow-50">
          ...
          <AlertDescription className="text-yellow-700 text-sm">
            Payment for this package has not been confirmed and delivery time is
            not yet up. Tracking information is not available.
          </AlertDescription>
        </Alert>
      )}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-sm print:text-xs">
                ...
                <Badge
                  className={`${getStatusColor(
                    packageData.status
                  )} px-3 py-1 text-xs font-medium border rounded-full print:text-xs print:px-2 print:py-0.5`}
                  variant="secondary"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 print:gap-2">
                ...
                <div className="md:col-span-4">
                  <div className="flex items-center justify-between mb-1 print:mb-0.5">
                    <span className="text-xs font-medium text-gray-600 print:text-xs">
                      Delivery Progress
                    </span>
                    <span className="text-xs font-bold text-blue-600 print:text-xs">
                      {progress}% Complete
                    </span>
                  </div>
                  <Progress
                    value={Math.max(0, Math.min(100, progress))}
                    className="h-2 print:h-1.5"
                  />
                </div>
                ...
                <div className="space-y-2 print:space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs font-medium text-gray-600 print:text-xs">
                        Package ID
                      </p>
                      <p className="font-mono font-medium text-base print:text-sm">
            ...
            <div className="lg:col-span-2 space-y-6 print:space-y-4">
              ...
              <Card className="rounded-xl border shadow-md print:border-none print:shadow-none">
                ...
                <CardContent className="p-0 print:hidden">
                  {" "}
                  {/* Hide map on print, as it may not fit well on paper */}
                ...
                <CardContent className="hidden print:block pt-2">
                  {" "}
                  {/* Print-friendly alternative for map */}
                  <p className="text-sm text-gray-600 italic">
                    Map view not available in print mode. Please view online for
                    interactive map.
                  </p>
                </CardContent>
              </Card>
                  ...
                  <div className="grid grid-cols-2 gap-3 print:gap-2">
                    ...
                    {packageData.delivered_at && (
                      <div>
                        ...
                        <p className="text-sm print:text-xs">
                          {formatDate(packageData.delivered_at)}
                        </p>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="pt-3 border-t print:pt-2">
                      ...
                      <div className="space-y-2 print:space-y-1">
                        {locationHistory.map((loc, idx) => (
                          <div
                            key={idx}
                            className="flex items-start gap-2 p-2 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-md border border-blue-100 print:p-1 print:text-xs print:border-none print:bg-none"
                          >
        ...
        <Card className="rounded-xl border shadow-md text-center py-12 print:py-8">
          <CardContent>
            <div className="max-w-md mx-auto">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-2 print:text-lg print:mb-1">
                No Package Found
              </h3>
              <p className="text-gray-600 mb-4 text-sm print:text-xs print:mb-2">
                We couldn't find a package with that tracking ID. Please check
                your package ID and try again.
              </p>
              <Button
